...
import type { OddsData, GameData, PlayerStatsData } from '@/services/SportsRadarService';
...
export const SportsRadarTestPage: React.FC = () => {
  ...
  const runAllTests = async () => {
    ...
    await runTest('Player Stats (LeBron James)', async () => {
      ...
      return await sportsRadarService.getPlayerStats('nba', 'player-lebron-james-id');
    });
  ...
  return (
    <Box p={3}>
      ...
      <Alert severity="info" sx={{ mb: 3 }}>
        This page tests all SportsRadar API endpoints to ensure proper integration.
                    ...
                    <Box>
                      {test.name === 'Health Check' && (
                        <Box>
                          ...
                          <List dense>
                            {test.data.availableAPIs?.map((api: string, i: number) => (
                              <ListItem key={i}>
                      ...
                      {(test.name.includes('NBA Games') || test.name.includes('Odds')) && (
                        <Box>
                          <Typography variant="subtitle2" gutterBottom>
                            Found {Array.isArray(test.data) ? test.data.length : 'N/A'} items
                          </Typography>
                          {Array.isArray(test.data) && test.data.slice(0, 3).map((item: any, i: number) => (
